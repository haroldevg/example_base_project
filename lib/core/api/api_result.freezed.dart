...
class _$ApiResultTearOff {
  ...
  Failure<T> failure<T>({required NetworkException networkException}) {
    return Failure<T>(
...
class _$Failure<T> implements Failure<T> {
  const _$Failure({required this.networkException});
...
abstract class Failure<T> implements ApiResult<T> {
  const factory Failure({required NetworkException networkException}) =
      _$Failure<T>;
